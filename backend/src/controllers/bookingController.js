...
exports.getAllBookings = async (req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT b.*, r.name AS room_name, r.capacity AS room_capacity, u.name AS unit_name
       FROM bookings b
...
exports.createBooking = async (req, res) => {
  ...
  try {
    ...
    const [unitRows] = await pool.query('SELECT id FROM units WHERE id = ?', [unit_id]);
    ...
    const [result] = await pool.query(
      `INSERT INTO bookings
        (room_id, unit_id, booking_date, start_time, end_time, participant_count, nominal, consumption_type)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        room_id,
        unit_id,
...
exports.updateBooking = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `UPDATE bookings SET
        room_id = ?, unit_id = ?, booking_date = ?, start_time = ?, end_time = ?,
